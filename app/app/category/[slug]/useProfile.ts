...
import { fetchProfile, Profile } from "./fetch";
...
export const useProfile = (pageSize = 10, category: string) => {
  const { user } = useAuth();
  const {
    data,
    ...
    queryKey: ["profiles", pageSize, category],
    ...
    queryFn: ({ pageParam }) => {
      ...
      return fetchProfile(pageSize, pageParam, category);
    },
